...
Value *stringToValue(char *s) {
    ...
    if (!v) {
        THROW(RC_ERROR, "Failed to allocate memory for Value.");
    }
    v->dt = DT_STRING;
    v->v.stringV = strdup(s);
    if (!v->v.stringV) {
        free(v);
        THROW(RC_ERROR, "Failed to allocate memory for string value.");
    }
    printf("Created Value with dt = %d and stringV = %s\n", v->dt, v->v.stringV);
    return v;
}
